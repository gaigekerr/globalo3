...
Revision History
    ...
    07112019 -- function 'open_merra2_cyclones' edited to also extract the 
                ...
                output dataframs
    24012019 -- reuse_weights argument to regridder added in function
                interpolate_merra_to_ctmresolution
"""
...
def open_merra2u_specifieddomain(years, varname, levmax, levmin, lngmin, 
    ...
    years : list
        Year or range of years in measuring period, [years,]
    varname : str
        Variable of interest from GSFC runs. Options include
        ps (surface pressure), u (wind), t (temperature), co50 (CO emissions
        over mainly Asia with a 50 day life time), co25 (same as co50 but with 
        a 25 day lifetime), nh50 (fixed mixing ratio at the surface between 
        30 and 50˚N with a 50 day lifetime), st80_25 (fixed mixing ratio in the 
    ...
    infiles = []
    for year in years:
        PATH_MERRA = '/Users/ghkerr/phd/meteorology/data/inst3_3d_asm_Np/%d/'%year
    ...
    ds = ds.assign_coords(lon=(ds.lon % 360)).roll(
        lon = (ds.dims['lon']//2-1), roll_coords = True)
    # Subset Dataset with respect to spatial coordinates 
    ds = ds[varname].sel(lat=slice(latmin, latmax), lon=slice(lngmin, lngmax),
        lev=slice(levmax,levmin))
    # Extract
    var = ds.values
    lat = ds.lat.values
    lng = ds.lon.values
